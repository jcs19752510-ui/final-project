...
>
> ⚠️ **이 요약 문서만 읽고 Phase A(§4)를 실행하지 마세요.** 실제로 강제되는
> 세부 규칙(파일명 규칙, 기준 브랜치=CI 일치 등)은 `harness_00_overview.md`
> 원문에 있고, 이미 반복됐던 실수는 `harness_06_meta_improvement.md` §3
> 실패 패턴 라이브러리에 등록돼 있습니다. Phase A 착수 전 **원문 SOP
> (`00_overview`)와 §3 실패 패턴 표를 반드시 먼저 훑고 시작하세요** — 이
> 경고는 요약만 보고 착수했다가 이미 등록된 실수를 그대로 재현한 실제
> 사례(2026-09-07) 이후 추가되었습니다.
>
