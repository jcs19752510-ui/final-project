...
| F-002 | 멀티모달 입력: 텍스트(코드)+오디오(답변)+이미지(화이트보드/표정 프레임) | 원안(REQ-F-002)에서 "실시간 동시 처리"를 "턴 단위 순차 업로드"로 축소 | Must → **Should로 하향 조정**(2026-09-08) | 텍스트(코드)·오디오는 구현·검증 완료. **2026-09-09 갱신**: 이미지 입력도 U2-c(화이트보드)/U3-b(표정 프레임) 둘 다 구현·검증 완료로 전부 채워짐(U2-c는 실제 Docker+실제 Gemini Vision API로 curl 검증, `내부테스트결과서/` 참조) |
...
| F-005 | 시스템 설계 화이트보드: 캔버스로 그린 이미지를 Gemini Vision으로 평가 | 원안(REQ-F-005) 유지, GPT-4V→Gemini Vision | Could | **2026-09-09 갱신**: 구현·검증 완료(`docs/trd/aimock_u2c_trd.md`). 텍스트 대화 LLM은 Groq로 전환됐지만(ADR-002/ADR-008) 화이트보드는 독립 기능이라 사용자가 Gemini Vision을 명시적으로 유지 선택. 실제 이미지 업로드→AES-256-GCM 암호화 저장→Gemini Vision 평가→응답까지 실제 Docker 컨테이너+실제 Gemini API로 검증됨(415/413/403 에러 케이스 포함, AC-1~6 전부 pytest+실측 curl 통과) |
