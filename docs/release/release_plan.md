...
| 주차 | 목표 | 포함 단위 | 상태 |
|---|---|---|---|
| Week 1 | 기반 구축: 프로젝트 스켈레톤, DB 스키마, 인증, 탈퇴 소프트삭제+30일 파기 스케줄러(APScheduler), 원본 미디어 암호화 저장+삭제 API, LLM/STT 어댑터 PoC | U1, U1-b, (U3-a PoC) | 예정 |
| Week 2 | 핵심 루프 완성: 턴 기반 질문-답변, RAG 질문은행, 채점 JSON 스키마 | U2-a, U3-a | 예정 |
| Week 3 | 기술 평가 + 비언어 분석: 라이브 코딩, DeepFace/librosa 통합 | U2-b, U3-b | 예정 |
| Week 4 | 리포트/대시보드 + QA + (여유 시) 화이트보드 + 데모 준비 | U4, U5, (U2-c), QA | 예정 |
