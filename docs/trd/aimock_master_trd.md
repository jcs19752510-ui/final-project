...
| F-006 | 상세 피드백 리포트: STAR 구조 분석, 핵심 키워드, 표정 타임라인 | 원안(REQ-F-006) 유지, "시선처리"는 프레임 표정분석 범위로 축소(눈동자 추적 아님). **2026-09-08: "발화속도" 항목 삭제**(구현된 적 없어 요구사항에서 제외, 필요 시 별도 단위로 재검토) | Must | STAR 분석·키워드는 구현·검증 완료(실제 Gemini로도 확인). 표정 타임라인은 U3-b 미착수라 항상 빈 배열 |
